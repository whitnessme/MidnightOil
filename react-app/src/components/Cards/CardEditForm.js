...
const CardEditForm = ({ setShowModal, cardId }) => {
  ...
  useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        e?.preventDefault();
        const data = await dispatch(editACard(deckId, {front, back, deck_id: +deckId}))
        if (data.errors) {
            setErrors(data)
        } else {
            setShowModal(false)
            ...
            <form className='create-deck-form' onSubmit={handleSubmit}>
                ...
                <div className='input-label-div select-div'>
                    ...
                    <ul name="rating" className='select-confidence-ul'>
                        <li className='rating-option rating-1' onClick={() => setRating(1)}>1</li>
                        <li className='rating-option rating-2' onClick={() => setRating(2)}>2</li>
                        <li className='rating-option rating-3' onClick={() => setRating(3)}>3</li>
                        <li className='rating-option rating-4' onClick={() => setRating(4)}>4</li>
                        <li className='rating-option rating-5' onClick={() => setRating(5)}>5</li>
                    </ul>
                </div>
